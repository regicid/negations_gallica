...
custom_var_computer <- function(ratio, total, b_hat_T, b_hat_R, ess, conf_int = 0.5) {
  T = total
  m_90 = ratio + qnorm(conf_int, mean = 0, sd = 1) * sqrt(ratio*(1-ratio)/(ess*T))
  V_X = m_90*(1-m_90)
  V_R = (ess * V_X / T) + (ess**2) * (b_hat_T*(1/log(T)) + b_hat_R*V_X)
  return(V_R)
}
cov_ij_computer <- function(var, ess, ratio, total) {
  cov_ij = (1/(ess**2)) * (var - ((ess**2)*ratio*(1-ratio)/total))
  return(cov_ij)
}

ess = quantile(reg_data$ratio, 0.9)
...
b_hat_T_975 = confint(reg, level = 0.99)['I(1/log(total))', '99.5 %']
b_hat_R_025 = confint(reg, level = 0.99)['ratio', '0.5 %']
